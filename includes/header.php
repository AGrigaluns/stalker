...
<nav id="navBar" class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        ...
                        <div class="dropdown-menu">
                            <ul>
                                ...
                                <?php foreach ($entry['dropdown'] as $subEntry) : ?>
                                    <a href="<?= $entry['slug'] ?>.html?type=<?= $subEntry['url'] ?>"
                                       class="dropdown-item"><?= $subEntry['label'] ?></a>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                    </li>
        ...
        <?php if (isset($_SESSION['user']) && isset($_SESSION['user']['username']) && !empty($_SESSION['user']['username'])) : ?>
            <?= $_SESSION['user']['username'] ?>
            <button class="btn btn-outline-primary my-2 my-sm-0" id="logoutBtn">
                <i class="fas fa-sign-out-alt"></i>
            </button>
        <?php else : ?>
            <button id="signInNav" class="btn btn-outline-primary my-2 my-sm-0"><a href="registration.php">Sign in</a>
            </button>
        <?php endif ?>
    </div>
</nav>
